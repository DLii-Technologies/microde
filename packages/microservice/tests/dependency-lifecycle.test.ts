...
class FixtureModule extends MicroserviceModule {
	...
	override async initialize(): Promise<void> { this.record('initialize'); }
	override async setup(): Promise<void> {
		this.record('setup');
		if (this.failSetup) throw new Error(`setup:${this.name}`);
	}
	override async run(): Promise<void> { this.record('run'); }
	override async stop(): Promise<void> { this.record('stop'); }
	override async teardown(): Promise<void> { this.record('teardown'); }
	override async shutdown(): Promise<void> { this.record('shutdown'); }
	override async cleanup(): Promise<void> { this.record('cleanup'); }
}
...
async function trace(
	...
	for (const name of permutation) {
		const handle = service.install(name, context =>
			new FixtureModule(
				context,
				name,
				events,
...
describe('dependency lifecycle fixtures', () => {
	const fixtures = [
		{ names: ['a', 'b', 'c'], edges: [['a', 'b'], ['b', 'c']] as Edge[], order: ['c', 'b', 'a'] },
		{ names: ['a', 'b', 'c'], edges: [['a', 'c'], ['b', 'c']] as Edge[], order: ['c', 'a', 'b'] },
		{ names: ['a', 'b', 'c', 'd'], edges: [['a', 'b'], ['a', 'c'], ['b', 'd'], ['c', 'd']] as Edge[], order: ['d', 'b', 'c', 'a'] },
		{ names: ['a', 'b', 'c'], edges: [['a', 'b'], ['a', 'c', 'reference'], ['c', 'a', 'reference']] as Edge[], order: ['b', 'a', 'c'] },
		{ names: ['orders', 'reports', 'primary', 'analytics'], edges: [['orders', 'primary'], ['reports', 'analytics']] as Edge[], order: ['analytics', 'primary', 'orders', 'reports'] },
	];

	for (const fixture of fixtures) {
		it(`is installation-order independent for ${fixture.names.join('-')}`, async () => {
			const forward = await trace(fixture.names, fixture.edges, fixture.names);
