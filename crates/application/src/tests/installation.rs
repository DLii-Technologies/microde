...
fn installs_passive_and_active_modules_in_order_with_the_shared_context() {
    ...
    let mut application = MicrodeApplication::with_context(context.clone());
    ...
    assert_eq!(application.state(), MicrodeApplicationState::Idle);
    assert_eq!(
        application
            .modules
            .iter()
    ...
    for installed in &mut application.modules {
...
fn rejects_installation_after_execution_has_started() {
    let context: MicrodeContextHandle = Arc::new(TestContext);
    let mut application = MicrodeApplication::with_context(context);
    application.set_state(MicrodeApplicationState::Running);
    let received_contexts = Arc::new(Mutex::new(Vec::new()));
    ...
    let error = application
        .install(passive_factory(received_contexts.clone()))
        .unwrap_err();
    let active_error = application
        .install(active_factory(received_contexts))
        .unwrap_err();
    let named_error = application
...
fn restores_idle_state_when_a_factory_panics() {
    ...
    let panic = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let _ = application.install(panicking_factory());
    }));
    ...
    application.set_state(MicrodeApplicationState::Running);
    assert!(application.install(panicking_factory()).is_err());
}
...
fn named_installation_returns_an_opaque_stable_handle() {
    ...
    let first = application.install_named("first", |_| Passive).unwrap();
    let second = application.install_named("second", |_| Passive).unwrap();
...
fn duplicate_named_installation_is_rejected_before_factory_evaluation() {
    let context: MicrodeContextHandle = Arc::new(TestContext);
    let mut application = MicrodeApplication::with_context(context);
    application.install_named("worker", |_| Passive).unwrap();
